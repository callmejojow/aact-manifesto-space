        ...
        <div class="snap-y snap-mandatory h-screen overflow-scroll">
            <div class="sticky z-30 top-0 bg-ivory h-32 lg:h-28 pt-4 md:pt-6 lg:pt-4 px-6 md:px-12 lg:px-0">
                ...
                <nav ref="navbar" class="lg:hidden bg-ivory t-0 border-b border-stone-600 flex space-x-5 overflow-x-auto max-w-screen text-base">
                    ...
                    <a v-for="(artist, index) in bodyPoliticsArray" :key="'nav_item_' + index" :href="`#artist_${index}`" class="nav-item inline-block whitespace-nowrap" :class="{ 'font-bold text-stone-800': activeIndex == index + 1, 'text-stone-500': activeIndex != index + 1 }">
                        {{artist.artist_name}}
                    </a>
                </nav>
            </div>
            <!-- Banner -->
            <section id="page_banner" class="scroll-mt-32 lg:scroll-mt-28 snap-start h-screen min-h-screen w-full bg-cover bg-top banner1-url overflow-hidden z-20">
