        ...
        <div class="snap-y snap-mandatory">
            <div class="sticky top-0 z-30 bg-ivory h-36 lg:h-28">
                ...
                <nav ref="navbar" class="lg:hidden bg-ivory t-0 border-b border-stone-600 flex space-x-5 mx-4 overflow-x-auto max-w-screen">
                    ...
                    <a v-for="(artist, index) in bodyPoliticsArray" :key="'nav_item_' + index" :href="`#artist_${index}`" class="nav-item inline-block whitespace-nowrap text-stone-600 text-sm" :class="{ 'font-bold': activeIndex == index + 1 }">
                        {{artist.artist_name}}
                    </a>
                </nav>
                <br class="lg:hidden">
